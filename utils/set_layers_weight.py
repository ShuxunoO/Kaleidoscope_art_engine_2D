...
def divide_dirs_into_beacon_and_subordinate(layerconfig_json, layerinfo_json):
    layers_order = layerconfig_json["layersOrder"]
    for layer in layers_order:
        layer_name = layer["name"]
        layer_info = layerinfo_json[layer_name]
        if layer_info["existSubdir"] == True: # 在含有子文件夹的情况下再进行分类
            dir_list = layer_info["dir_list"]
            # 纯信标层
            if layer_info["isBeaconLayer"] == True and "groupBy" not in layer_info:
                layer_info["beacon_dir_list"] = dir_list
            # 纯附属子层
            elif layer_info["isBeaconLayer"] == False and "groupBy" in layer_info:
                layer_info["subordinate_dir_list"] = dir_list
            # 既是信标层，又是附属子层
            else:
                ...
                # 两个列表取交集得到交集附属子文件夹列表
                layer_info["subordinate_dir_list"] = list(
                    set(dir_list).intersection(set(beacon_dir_list)))
                # 两个列表取差集得到新的信标文件夹列表
                layer_info["beacon_dir_list"] = list(set(dir_list).difference(set(beacon_dir_list)))
...
def balance_layer_weight_in_subdirs(layerinfo_json, _SUM, layer_name):
    ...
    if layer_info["isBeaconLayer"] == True and "groupBy" not in layer_info:  # 只是信标层
        dir_list = layer_info["beacon_dir_list"]
        ...
        dir_list = layer_info["subordinate_dir_list"]
...
def redistribute_beacon_subordinate_layer(layerinfo_json, _SUM, layer_name):
    layer_info = layerinfo_json[layer_name]
    dir_list = layer_info["dir_list"]
    beacon_dir_list = layer_info["beacon_dir_list"]
    subordinate_dir_list = layer_info["subordinate_dir_list"]
    # 先分配附属子层的文件夹
